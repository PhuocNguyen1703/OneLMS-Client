...
export const filterTables = (
  ...
  return tables.filter((table) => {
    const matchStatus =
      activeStatuses.length === 0 || activeStatuses.includes(table.status);

    const matchSeat =
      activeSeats.length === 0 || activeSeats.includes(String(table.seatCount));

    const matchArea =
      activeAreas.length === 0 || activeAreas.includes(table.area);
